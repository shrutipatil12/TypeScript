...
export class Inventory {
    /**
     * @param: pname,weight,price,totalprice
     */
    pname: string;
    weight: number;
    price: number;
    ...
    public customer(): number {
        try {
            ...
            let choice = readlineSync.question('Enter your choice: ')  //reading user input
            if (choice < 1 || choice > 2) {
                console.log('please enter correct choice')
            }
            else {
                switch (choice) {                 //switch case for asking users choice
                    case '1': this.purchase()    //calling function
                        break;
    ...
    public purchase(): void {
        try {
            ...
            var ch:number = readlineSync.question('1.Sugar\n2.Rice\n3.Wheat')    //reading user input
